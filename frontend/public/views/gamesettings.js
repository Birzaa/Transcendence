...
export function renderGameSettings() {
    ...
    // si parametres deja presents-> lancer la partie
			...
			<div class="min-h-screen flex items-center justify-center">
				<div class="max-w-lg w-full bg-pink-50 bg-opacity-95 shadow-lg border-2 border-purple-300">
					<div class="bg-purple-600 text-pink-100 p-3">
						<h1 class="text-xl font-bold text-center">Paramètres du mode ${mode === "1v1" ? "1 vs 1" : "Solo"}</h1>
					</div>
					<div class="p-6">
						...
						<!-- Raquette joueur 1 -->
						<div> 
							<h3 class="text-purple-600 font-bold text-center mb-2">Raquette Joueur 1</h3>
							<div class="pt-4 flex justify-center gap-2 flex-wrap">
								${["bleu", "vert", "jaune", "orange", "rose", "rouge", "violet", "gris"].map(c => `
									<label>
										...
										<img src="/images/raquette_${c}.png"
													class="paddle-option cursor-pointer w-7 h-24 border-2 ${c === "gris" ? "border-purple-400" : "border-transparent"} rounded" />
									</label>
								`).join("")}
							</div>
						</div>
						...
						${mode === "1v1" ? `
						<div>
							<h3 class="text-purple-600 font-bold text-center mb-2">Raquette Joueur 2</h3>
						...
						<div class="flex items-center justify-center gap-3">
							<label for="score" class="text-purple-600 font-bold text-center">Score à atteindre :</label>
							<input id="score" name="score" type="number" value="5" min="1" max="20"
								class="px-2 py-1
								relative bg-purple-200 border-2 border-t-white border-l-white border-r-purple-400 border-b-purple-400 
    ...
    document.querySelectorAll("input[name='color1'], input[name='color2']").forEach(input => {
    ...
    form.addEventListener("submit", (e) => {
        ...
        const color1 = formData.get("color1") || "rose";
        const color2 = mode === "1v1" || mode === "tournament" ? (formData.get("color2") || "bleu") : "";
