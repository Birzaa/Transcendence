...
export function renderRemote() {
    ...
    let role = url.searchParams.get('role') || 'host';
    if (!room) {
...
function initRemoteGame(room, role) {
    // ---- DOM
            ...
            ws.onmessage = (ev) => {
                try {
                    ...
                    if (msg.type === 'state' && role === 'guest') {
                        ...
                        gameWidth = st.gameWidth;
                        gameHeight = st.gameHeight;
                        draw();
                    }
                }
                catch { }
    ...
    function sendInput() {
        ...
        const msg = { type: 'input', room, up, down };
        ws.send(JSON.stringify(msg));
    }
    function sendServe() {
        if (!ws || !wsConnected)
            return;
        const msg = { type: 'serve', room };
        ws.send(JSON.stringify(msg));
    }
