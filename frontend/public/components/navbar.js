...
import { setLanguage, getLanguage, updateUI } from "../utils/i18n.js";
/**
 * Crée la barre de navigation selon l'état de connexion
 */
export async function navBar() {
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 py-4 bg-pink-200 shadow-md">
      ...
      <div class="flex items-center space-x-6 mr-6">
          <a href="/" data-i18n="Home" class="relative px-4 py-2 bg-pink-200 border-2 border-t-white border-l-white border-r-pink-400 border-b-pink-400 text-blue-300 font-bold shadow-[2px_2px_0px_0px_rgba(147,51,234,0.3)] active:shadow-none active:translate-y-[2px] active:border-pink-300 transition-all duration-100">Home</a>
          <a href="/chat" data-i18n="Chat" class="relative px-4 py-2 bg-pink-200 border-2 border-t-white border-l-white border-r-pink-400 border-b-pink-400 text-blue-300 font-bold shadow-[2px_2px_0px_0px_rgba(147,51,234,0.3)] active:shadow-none active:translate-y-[2px] active:border-pink-300 transition-all duration-100">Chat</a>
          <a href="/profil" data-i18n="Profile" class="relative px-4 py-2 bg-pink-200 border-2 border-t-white border-l-white border-r-pink-400 border-b-pink-400 text-blue-300 font-bold shadow-[2px_2px_0px_0px_rgba(147,51,234,0.3)] active:shadow-none active:translate-y-[2px] active:border-pink-300 transition-all duration-100">Profile</a>
          ${isLogin
        ? `<a href="/logout" id="logout-btn" data-i18n="Logout" class="relative px-4 py-2 bg-pink-200 border-2 border-t-white border-l-white border-r-pink-400 border-b-pink-400 text-blue-300 font-bold shadow-[2px_2px_0px_0px_rgba(147,51,234,0.3)] active:shadow-none active:translate-y-[2px] active:border-pink-300 transition-all duration-100">Logout</a>`
        : `<a href="/auth" data-i18n="Login" class="relative px-4 py-2 bg-pink-200 border-2 border-t-white border-l-white border-r-pink-400 border-b-pink-400 text-blue-300 font-bold shadow-[2px_2px_0px_0px_rgba(147,51,234,0.3)] active:shadow-none active:translate-y-[2px] active:border-pink-300 transition-all duration-100">Login</a>`}
          <div class="relative">
              <select id="language-select" class="relative px-4 py-2 bg-pink-200 border-2 border-t-white border-l-white border-r-pink-400 border-b-pink-400 text-blue-300 font-bold shadow-[2px_2px_0px_0px_rgba(147,51,234,0.3)] active:shadow-none active:translate-y-[2px] active:border-pink-300 transition-all duration-100 appearance-none pr-8">
                  <option value="fr">🇫🇷 Français</option>
                  <option value="en">🇬🇧 English</option>
                  <option value="es">🇪🇸 Español</option>
              </select>
              <span class="pointer-events-none absolute inset-y-0 right-2 flex items-center text-blue-300">▼</span>
          </div>
      </div>
      ...
      <style>
          ...
          .ping-pong-paddle-right { animation: paddleRight 3s ease-in-out infinite; }
  ...
  `;
    ...
    if (isLogin) {
        ...
        if (logoutBtn) {
            logoutBtn.addEventListener("click", async (e) => {
                ...
                if (window.socket)
                    window.socket.close();
                await connectWebSocket("anonymous");
                navigate("/");
                // mise à jour de la langue après logout
                updateUI();
    ...
    // Attacher le listener du select pour changer la langue
    const select = nav.querySelector("#language-select");
    if (select) {
        select.value = getLanguage();
