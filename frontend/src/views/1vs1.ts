...
function updateTournamentProgress(): void {
...
function getRoundNameForProgress(matchCount: number): string {
  if (matchCount === 1) return t("Tournament_Progress_Final");
  if (matchCount === 2) return t("Tournament_Progress_SemiFinal");
  if (matchCount === 4) return t("Tournament_Progress_QuarterFinal");
  return `${t("Tournament_Progress_Round")} (${matchCount} ${t("Tournament_Matches")})`;
}

function init1vs1Game(player1Name: string, player2Name: string, mode: string | null, WIN_SCORE: number)  {
  let player1Score = 0;
  let player2Score = 0;
  ...
  function goToNextMatch(winner: string) {
    ...
    const playersMap: { [name: string]: string } = JSON.parse(localStorage.getItem("tournamentPlayers") || "{}");
  ...
  function showRoundSummary(
    ...
    document.getElementById("continue-tournament")?.addEventListener("click", () => {
      const newRound: { p1: { name: string, color: string }, p2: { name: string, color: string } | null }[] = [];
