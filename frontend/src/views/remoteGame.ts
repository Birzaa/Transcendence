  ...
  function loop() {
    if (role === 'host') {
      ...
      if (!waitingForServe) {
        ...
        if (ballX < 0) { s2++; waitingForServe = true; }
        if (ballX > gameWidth) { s1++; waitingForServe = true; }
      }
    ...
    // --- Affichage ---
    (document.getElementById('paddle1') as HTMLElement).style.top = `${p1Y}px`;
    (document.getElementById('paddle2') as HTMLElement).style.top = `${p2Y}px`;
    (document.getElementById('ball') as HTMLElement).style.left = `${ballX}px`;
    (document.getElementById('ball') as HTMLElement).style.top = `${ballY}px`;
    (document.getElementById('player1-score') as HTMLElement).textContent = s1.toString().padStart(2, '0');
    (document.getElementById('player2-score') as HTMLElement).textContent = s2.toString().padStart(2, '0');
